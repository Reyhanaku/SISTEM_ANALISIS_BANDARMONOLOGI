        ...
        with st.form("log_book_form"):
            ...
            if submit_log:
                with st.spinner("Menyimpan ke Log Book..."):
                    log_payload = {
                        "action": "save_logbook", "emiten": st.session_state['emiten'],
...
with tab2:
    ...
    if refresh or 'logbook_data' not in st.session_state:
        with st.spinner("Mengambil data Log Book terbaru..."):
            try:
                logbook_response = requests.post(GAS_URL, json={"action": "get_logbook"}, allow_redirects=True)
    ...
    if 'logbook_data' in st.session_state:
        ...
        if not df_logbook.empty:
            ...
            st.info("📂 Log Book Anda masih kosong atau data belum ditarik. Silakan tekan tombol Refresh.")